...
static void input_callback(InputEvent* input_event, void* ctx) {
    ...
    TextEditorEvent event = {.type = EventTypeInput, .input = *input_event};
...
static void timer_callback(FuriMessageQueue* event_queue) {
    ...
    TextEditorEvent event = {.type = EventTypeTick};
...
int32_t text_editor_app(void* p) {
    ...
    // Текущее событие типа кастомного типа TextEditorEvent
    TextEditorEvent event;
    // Очередь событий на 8 элементов размера TextEditorEvent
    FuriMessageQueue* event_queue = furi_message_queue_alloc(8, sizeof(TextEditorEvent));
